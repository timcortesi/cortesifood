...
while(list( , $node) = each($result)) {
    ...
    if ($title == "Chili Con Queso") {
    //    var_dump($text);
    }
    /* Get All Images */
    $matches_img = [];
    preg_match_all('/\[.*?Image:(\w+\.?\w+) "wikilink"\)/s',$text,$matches_img);
    $matches_file = [];
    preg_match_all('/\[.*?File:(\w+\.?\w+) "wikilink"\)/s',$text,$matches_file);
    $all_img_matches = array_merge($matches_img[1],$matches_file[1]);
    foreach($all_img_matches as $img_index => $img_match) {
        $all_img_matches[$img_index] = strtolower($img_match);
    }
    $frontmatter .= "images: [".implode(', ',$all_img_matches)."]\n";

    /* Remove Original Image Text */
    $text = preg_replace( '/\[.*?Image:(\w+\.?\w+) "wikilink"\)/s', "", $text);
    $text = preg_replace( '/\[.*?File:(\w+\.?\w+) "wikilink"\)/s', "", $text);

    /* Get All Categories & Ingredeients */
    $list_of_ingredients = [
        'Apples',
        'Bananas',
        'Beef',
    ...
    $matches_categories = [];
    preg_match_all('/\[Category:(.*?)\].*? "wikilink"\)/s',$text,$matches_categories);
    $all_categories = [];
    $all_ingredients = [];
    foreach($matches_categories[1] as $mycategory) {
        if (in_array($mycategory, $list_of_ingredients)) {
            $all_ingredients[] = $mycategory;
        } else {
            $all_categories[] = $mycategory;
        }
    }
    $frontmatter .= "categories: [".implode(', ',str_replace("\n"," ",$all_categories))."]\n";
    $frontmatter .= "ingredients: [".implode(', ',str_replace("\n"," ",$all_ingredients))."]\n";

    /* Remove Original Category Text */
    $text = preg_replace( '/\[Category:(.*?)\].*? "wikilink"\)/s', "", $text);
